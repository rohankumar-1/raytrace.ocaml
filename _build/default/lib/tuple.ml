(*
NOTES:
- could rewrite as Record instead of class --> only components affected are is_point and is_vector, would need to be static 
  - this probably does not make sense since we would still need factory functions for point, vector
